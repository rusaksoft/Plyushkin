...
#            os.write(self.tempfile, "user %s %s\n" % (self.parsed_url.username, self.password))
    ...
    def list_with_attr(self):
        l = self._list_with_attr()

        return [x.split() for x in l.split('\n') if x]
...
duplicity.backend.uses_netloc.extend(['ftp', 'ftps', 'ftpes', 'fish',
                                      'lftp+ftp', 'lftp+ftps', 'lftp+ftpes',
                                      'lftp+fish', 'lftp+sftp',
